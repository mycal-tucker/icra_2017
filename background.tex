...
\begin{equation}
\boldsymbol{\phi}^* = \argmax_{\phi_{ij} \in \boldsymbol{\phi}} \prod_{i}^{\boldsymbol{|\lambda|}} \prod_{j}^{|\boldsymbol{\Gamma_i|}} \Psi(\phi_{ij},\gamma_{ij},\lambda_i,\boldsymbol{\Phi_{c_{i}}},\boldsymbol{\Upsilon_{KP}}),
\label{eq:llm1}
\end{equation}
...
Note that a limitation of the DCG model is that it assumes a fixed set of symbols (i.e., objects and phrases). Thus,  does not explicitly represent the unknown symbols (i.e., objects and phrases) as in Fig.~\ref{fig:dcg_plates}. Thus, this model is unable to reason about objects and phrases that have not been trained on. 